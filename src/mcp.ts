...
server.tool(
  ...
  async ({ width, height, background, actors, frames, delay, scale, palette: paletteId }) => {
    try {
      ...
      const actorLines = descriptions.map((desc) => `  - ${desc}`).join('\n');
      content.push({
        type: 'text' as const,
